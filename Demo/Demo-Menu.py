...
def on_drop(files):
    ...
    ground_truth = os.path.basename(os.path.dirname(file_path))
...
def on_predict():
    ...
    if breed == ground_truth:
        label.config(text=f"Prediction: {breed}", fg='green')
    else:
        label.config(text=f"Prediction: {breed}", fg='red')
